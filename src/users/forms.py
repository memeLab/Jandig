...
from django.contrib.auth.forms import PasswordChangeForm as OrigPasswordForm
...
class PasswordChangeForm(OrigPasswordForm):
    def __init__(self, *args, **kwargs):
...
class ProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # FIXME:
        # user.email come as a string of a tuple,
        # no idea why. "('email@bla.com',)"
        ...
        self.fields["bio"].widget.attrs["placeholder"] = _(
            "Personal Bio / Description"
        )
        self.fields["personal_site"].widget.attrs["placeholder"] = _(
            "Personal Website"
        )
...
class UploadMarkerForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["source"].widget.attrs["placeholder"] = _(
            "browse file"
        )
...
class ArtworkForm(forms.Form):
    ...
    marker_author = forms.CharField(
        max_length=12,
        required=False
    )
    augmented = forms.ImageField(required=False)
    augmented_author = forms.CharField(
        max_length=12,
        required=False
    )
...
class ExhibitForm(forms.Form):
    ...
    # Possible bug on max artworks per exhibit
    ...
    def clean_slug(self):
        ...
        if not re.match("^[a-zA-Z0-9_]*$", data):
            raise forms.ValidationError(
                _(
                    "Url can't contain spaces or special characters"
                )
            )
        return data
